...
def view_finished_water_good(request):
    ...
    return render(request,'production/view_finished_goods.html',{'view':page_obj,'cc':cc,'allocated_menus':allocated_menus})
...
def damaged_goods(request):
    ...
    return render(request, 'production/damaged_goods.html', {'allocated_menus':allocated_menus,'dd':page_obj})
...
def finishedgoods_production_search(request):
    ...
    categories = water_Finished_goods_category.objects.all()  # Get distinct categories
    ...
    if request.method == 'POST':
        ...
        if filters:
            search_list = water_Finished_Goods.objects.filter(filters)
...
def finishedgoods_production_stock_history(request, id):
    ...
    finished_good = Finished_Goods_Stock.objects.filter(finished_goods_id=id).order_by('-date')
...
def finished_goods_production_history_pdf(request, id):
    ...
    if parsed_date and parsed_date_end:
        ...
        data = Finished_Goods_Stock.objects.filter(finished_goods_id=id)
...
def damagedgoods_production_stock_history(request, id):
    ...
    damaged_goods = Damaged_Goods_Stock.objects.filter(damaged_id=id).order_by('-date')
...
def damaged_goods_production_history_pdf(request, id):
    ...
    if parsed_date and parsed_date_end:
        ...
        data = Damaged_Goods_Stock.objects.filter(damaged_id=id)
...
def damaged_goods_production_history_excel(request, id):
    ...
    if parsed_date and parsed_date_end:
        ...
        damaged_good = Damaged_Goods_Stock.objects.filter(damaged_id=id)
...
def finished_goods_message_history_excel(request):
    ...
    if parsed_date and parsed_date_end:
        ...
        finished_good = finished_good.annotate(date_only=Cast('date', DateField())).filter(date_only=parsed_date)
