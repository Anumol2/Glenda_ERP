...
def finishedgoods_request_messages_list(request):
    ...
    return render(request, 'inventory/finishedgoods_request_message_list.html', {'data': page_obj, 'allocated_menus': allocated_menus})
...
def finished_goods_allocate_excel(request, id):
    ...
    # Get initial queryset for allocated stock based on provided ID
    items = Finished_Goods_allocate.objects.filter(finished_good_id=id)
    ...
    # Write the header row
    headers = ['Category', 'Name', 'Size', 'Allocated Stock', 'Date']
    ws.append(headers)
...
def finished_goods_stock_allocate_pdf(request, id):
    ...
    if parsed_date and parsed_date_end:
        ...
        data = Finished_Goods_allocate.objects.filter(finished_good_id=id)
